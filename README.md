...
NapCatQQ + NoneBot2 的 QQ 日常聊天角色机器人。默认角色是《明日方舟》重岳，使用武汉话口吻的人设文件 `personas/default_dialect.yaml`，也保留普通话版本 `personas/default.yaml`；蕾缪安使用独立的 `personas/lemuen.yaml` 实例配置。
...
```text
QQ 小号（每个角色一个账号）
  -> 独立 NapCatQQ runtime / napcat@<instance>.service
  -> OneBot V11 reverse WebSocket ws://127.0.0.1:<instance-port>/onebot/v11/ws
  -> 独立 NoneBot / qq-rolebot@<instance>.service
  -> ChatService
...
Production can run multiple rolebot instances at the same time. Each instance has its own QQ
account, `BOT_PORT`, persona, SQLite database, caches, and NapCat profile; only deployed source,
Python dependencies, read-only model weights, and optional static sticker assets are shared. See
[`docs/deployment.md`](docs/deployment.md#multi-instance-production) for the instance layout and
...
```bash
conda create -n qq-rolebot-wsl python=3.11 -y
conda activate qq-rolebot-wsl
...
If `conda` is not installed in WSL yet, install Miniconda first:
...
"$HOME/miniconda3/bin/conda" create -n qq-rolebot-wsl python=3.11 -y
```
...
7. Restart every configured `qq-rolebot@<instance>.service`, or the legacy single service.
8. Wait for every configured instance port to accept connections.
...
NapCat says `ECONNREFUSED 127.0.0.1:<port>`:
...
- Check the matching `qq-rolebot@<instance>.service` logs.
